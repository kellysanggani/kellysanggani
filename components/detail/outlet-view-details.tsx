    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="text-xl sm:text-2xl font-bold break-words text-gray-800">{outlet.name} - Details</h1>
          <p className="text-sm text-gray-600">Last updated: {formatDateTime(outlet.updated_at)}</p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription className="text-gray-600">
            Current inventory quantities with product availability and expiration dates
          </CardDescription>
        </CardHeader>
        <CardContent>
              ...
              {currentStockLevels.map((stock) => {
                ...
                return (
                  <div key={stock.product_id} className="space-y-2 text-center p-4 border rounded-lg">
                    <h4 className="font-medium text-sm sm:text-base break-words text-gray-800">{stock.product_name}</h4>
                    <div className="text-2xl sm:text-3xl font-bold text-gray-900">{stock.quantity}</div>
                    <p className="text-xs sm:text-sm text-gray-600">units in stock</p>
                    ...
                    {product?.expiration_date && (
                      <div className="space-y-1">
                        <p className="text-xs font-medium text-gray-700">Expiration Date:</p>
                        <p className="text-xs text-orange-700">{formatDate(product.expiration_date)}</p>
                      </div>
                    )}

                    {stock.updated_by && <p className="text-xs text-gray-600">Last updated by {stock.updated_by}</p>}
                  </div>
                )
              })}
            </div>
          ) : (
            <p className="text-sm text-gray-600 text-center py-8">No stock information available for this outlet.</p>
          )}
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription className="text-gray-600">Stock and order pictures for this outlet</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid gap-6 grid-cols-1 md:grid-cols-2">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                <Label className="text-sm font-medium text-gray-700">Stock Picture</Label>
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center">
                  {stockPicture ? (
                    <div className="space-y-2">
                      ...
                      />
                      <p className="text-xs text-gray-600">Stock picture uploaded</p>
                    </div>
                  ) : (
                    <div className="space-y-2">
                      <ImageIcon className="h-12 w-12 text-gray-400 mx-auto" />
                      <p className="text-sm text-gray-600">No stock picture uploaded</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                <Label className="text-sm font-medium text-gray-700">Order Picture</Label>
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center">
                  {orderPicture ? (
                    <div className="space-y-2">
                      <img
                        ...
                        className="max-w-full h-32 object-cover mx-auto rounded"
                      />
                      <p className="text-xs text-gray-600">Order picture uploaded</p>
                    </div>
                  ) : (
                    <div className="space-y-2">
                      <ImageIcon className="h-12 w-12 text-gray-400 mx-auto" />
                      <p className="text-sm text-gray-600">No order picture uploaded</p>
                    </div>
                  )}
                </div>
              </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription className="text-gray-600">Last check and order dates</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid gap-4 grid-cols-1 sm:grid-cols-2">
            <div className="space-y-2">
              <span className="text-sm font-medium text-gray-700">Last Check Date</span>
              <p className="text-sm break-words text-gray-800">{formatDate(outlet.last_check_date)}</p>
              <p className="text-xs text-gray-600">(Automatically updated when stock is updated)</p>
            </div>
            <div className="space-y-2">
              <span className="text-sm font-medium text-gray-700">Last PO Issue Date</span>
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
                <p className="text-sm break-words text-gray-800">{formatDate(lastOrderDate)}</p>
                <Button
                  size="sm"
                  variant="outline"
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg sm:text-xl text-gray-800">Sales Information</CardTitle>
          <CardDescription>
            <p className="text-xs text-gray-600">Assigned sales person for this outlet</p>
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            <div className="flex flex-col sm:flex-row items-start justify-between gap-4">
              <div className="space-y-2 flex-1">
                ...
                {isEditingSalesPerson ? (
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
                    <p className="text-sm font-medium break-words text-gray-800">{salesPerson || "Not assigned"}</p>
      ...
      {outlet.last_updated_by && outlet.last_updated_at && (
        <Card>
          <CardHeader>
            ...
            <CardDescription className="text-gray-600">
              Information about the most recent update to this outlet
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4 text-sm">
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium break-words text-gray-800">{outlet.last_updated_by}</span>
              </div>
              <span className="text-gray-600 hidden sm:inline">•</span>
              <span className="text-gray-600">{formatDateTime(outlet.last_updated_at)}</span>
            </div>
          </CardContent>
        </Card>
      )}
        ...
        <CardHeader>
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              ...
              <CardDescription className="text-gray-600">
                Complete history of changes made to this outlet
              </CardDescription>
            </div>
              ...
              {auditTrail.length > 0 ? (
                auditTrail.map((entry) => (
                  <div key={entry.id} className="border-l-2 border-muted pl-4 pb-4">
                    ...
                    <p className="text-sm text-gray-700 mb-1 break-words">{entry.details}</p>
                    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-1 sm:gap-2 text-xs text-gray-600">
                ...
                <p className="text-sm text-gray-600 text-center py-4">No audit history available for this outlet.</p>
              )}
            </div>
          </CardContent>
        )}
